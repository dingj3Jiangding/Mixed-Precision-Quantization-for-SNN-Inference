...
from .data import build_cifar10_dvs_loaders
from .metrics import (
    SpikeRateTracker,
    model_size_mb,
    parameter_count,
...
def direct_encode(x: torch.Tensor, t_steps: int) -> torch.Tensor:
    if x.ndim != 5:
        raise ValueError("Expected framed CIFAR10-DVS batch shape [B, T, C, H, W].")
    if x.shape[1] != t_steps:
        raise ValueError(
            f"Input has {x.shape[1]} time steps, but config requests {t_steps}. "
            "Regenerate framed CIFAR10-DVS data with a matching t_steps value."
        )
...
def run_baseline(cfg: BaselineCIFAR10DVSConfig) -> dict:
    ...
    last_checkpoint_path = output_dir / "fp32_last.pt"
    best_checkpoint_path = output_dir / "fp32_best.pt"
