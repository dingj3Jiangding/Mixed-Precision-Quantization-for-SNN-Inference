# 文件：`baseline_nmnist/weight_state_mixed.py`
...
如果你已经跑过 `baseline_nmnist` 的 Hessian-only 结果，推荐直接复用权重位宽分配，跳过最耗显存的 Hessian 二阶梯度：
```bash
python scripts/run_weight_state_mixed_vgg16.py \
  --checkpoint-path outputs/baseline_nmnist/fp32_last.pt \
...
## 输出文件
- `outputs/baseline_nmnist_weight_state_mixed/weight_bit_allocation.csv`
- `outputs/baseline_nmnist_weight_state_mixed/state_bit_allocation.csv`
- `outputs/baseline_nmnist_weight_state_mixed/comparison.csv`
- `outputs/baseline_nmnist_weight_state_mixed/weight_state_quant_finetune_epoch_metrics.csv`
- `outputs/baseline_nmnist_weight_state_mixed/summary.json`
- `outputs/baseline_nmnist_weight_state_mixed/weight_sensitivity_ranking.png`
