...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run state-aware Hessian mixed-precision verification on baseline_full "
            "without changing the Hessian-only validation path."
        )
    )
    parser.add_argument("--checkpoint-path", default="outputs/baseline_full/fp32_last.pt")
    parser.add_argument("--data-root", default="baseline/data")
    parser.add_argument("--output-dir", default="outputs/baseline_full_state_aware_hessian")
    ...
    parser.add_argument("--state-aware-alpha", type=float, default=0.75)
...
def main() -> None:
    ...
    from baseline_full.config import BaselineFullConfig
    from baseline_full.state_aware_hessian import run_state_aware_hessian_analysis
    ...
    summary = run_state_aware_hessian_analysis(
        cfg=cfg,
