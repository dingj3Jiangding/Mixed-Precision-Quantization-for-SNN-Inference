...
# Ensure repo root is importable when executing "python scripts/run_baseline_cifar100.py".
REPO_ROOT = Path(__file__).resolve().parents[1]
if str(REPO_ROOT) not in sys.path:
    sys.path.insert(0, str(REPO_ROOT))

from baseline_cifar100 import BaselineCIFAR100Config, run_baseline
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run reproducible VGG-16-like CIFAR-100 SNN baseline.")
    parser.add_argument("--data-root", default="baseline_cifar100/data")
    parser.add_argument("--output-dir", default="outputs/baseline_cifar100")
