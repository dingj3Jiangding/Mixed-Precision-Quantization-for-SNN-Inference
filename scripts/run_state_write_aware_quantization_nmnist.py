...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run state-write-aware quantization verification on baseline_nmnist."
    )
    parser.add_argument("--checkpoint-path", default="outputs/baseline_nmnist/fp32_best.pt")
    parser.add_argument("--data-root", default="baseline_nmnist/data")
    parser.add_argument("--output-dir", default="outputs/state_write_aware_quantization_nmnist")
    parser.add_argument("--state-bits", type=int, default=8)
    parser.add_argument("--relative-delta-threshold", type=float, default=0.05)
    parser.add_argument("--write-warmup-steps", type=int, default=4)
...
def main() -> None:
    ...
    from state_write_aware_quantization_nmnist.runner import run_state_write_aware_quantization_analysis
    ...
    summary = run_state_write_aware_quantization_analysis(
        cfg=cfg,
        checkpoint_path=args.checkpoint_path,
        state_bits=args.state_bits,
        relative_delta_threshold=args.relative_delta_threshold,
        write_warmup_steps=args.write_warmup_steps,
        output_dir=args.output_dir,
