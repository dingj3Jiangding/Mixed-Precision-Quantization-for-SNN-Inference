...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run a simplified joint weight+state mixed-precision experiment "
            "on baseline_full."
        )
    )
...
def main() -> None:
    ...
    from baseline_full.weight_state_mixed import run_weight_state_mixed_analysis

    weight_bits_list = _parse_bits(args.weight_bits)
    state_bits_list = _parse_bits(args.state_bits)
    ...
    summary = run_weight_state_mixed_analysis(
        cfg=cfg,
