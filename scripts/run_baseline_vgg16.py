...
# Ensure repo root is importable when executing "python scripts/run_baseline_vgg16.py".
REPO_ROOT = Path(__file__).resolve().parents[1]
if str(REPO_ROOT) not in sys.path:
    sys.path.insert(0, str(REPO_ROOT))

from baseline_vgg16 import BaselineVGG16Config, run_baseline
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run reproducible VGG-16-like CIFAR-10 SNN baseline.")
    parser.add_argument("--data-root", default="baseline/data")
    parser.add_argument("--output-dir", default="outputs/baseline_vgg16")
