...
def direct_encode(x: torch.Tensor, t_steps: int) -> torch.Tensor:
    if x.ndim != 5:
        raise ValueError("Expected framed N-MNIST batch shape [B, T, C, H, W].")
    if x.shape[1] != t_steps:
        raise ValueError(
            f"Input has {x.shape[1]} time steps, but config requests {t_steps}. "
            "Regenerate framed N-MNIST data with a matching t_steps value."
        )
...
def run_baseline(cfg: BaselineNMNISTConfig) -> dict:
