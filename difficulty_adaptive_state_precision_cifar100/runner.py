...
from baseline_cifar100.config import BaselineCIFAR100Config
from baseline_cifar100.data import build_cifar100_loaders
from baseline_cifar100.hessian import _iter_with_limit, _load_checkpoint, _quantize_tensor_symmetric_per_tensor
from baseline_cifar100.metrics import (
    SpikeRateTracker,
    model_size_mb,
    parameter_count,
    sop_proxy,
...
from baseline_cifar100.model import build_model
from baseline_cifar100.unquant_runner import direct_encode, set_global_seed, write_epoch_metrics_csv
from baseline_cifar100.weight_state_mixed import estimate_state_cost_proxy
...
def run_difficulty_adaptive_state_precision_analysis(
    cfg: BaselineCIFAR100Config,
    checkpoint_path: str,
    low_state_bits: int,
    high_state_bits: int,
    warmup_steps: int = 8,
    difficulty_metric: str = "confidence",
    easy_threshold: float = 0.9,
    output_dir: str = "outputs/difficulty_adaptive_state_precision_cifar100",
    ...
    train_loader, test_loader = build_cifar100_loaders(cfg, device=device)

    fp32_model = build_model(num_classes=100).to(device)
